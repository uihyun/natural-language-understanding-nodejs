...
import { analyzeWithAllFeaturesWithModel } from './utils/request';
...
export default React.createClass({
  ...
  onSubmitClick(value) {
    ...
    console.log("analyzeWithAllFeatures");
  ...
  onSubmitClickWithModel(value, model) {
    const query = this.state.requestType === 'url' ? { url: value } : { text: value };
    this.setState({
      query,
      disableButton: true,
    ...
    console.log("analyzeWithAllFeaturesWithModel");
    // Send the request to NLU with custom model id
    analyzeWithAllFeaturesWithModel(query, model)
    .then(data => this.setState({ data, loading: false, error: null }))
    .catch(error => this.setState({ error, loading: false }))
    .then(() =>
      setTimeout(() => { scrollToElement('#anchor', { duration: 300 }, 100); }, 0),
    ...
    return (
      <div className="_container _container_large">
        <Input
          ...
          onSubmitWithModel={this.onSubmitClickWithModel}
          onTabChange={this.enableButton}
          onInputChange={this.enableButton}
          onInputChangeTrue={this.enableButtonTrue}
